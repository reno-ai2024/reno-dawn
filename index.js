...
const createAxiosInstance = (proxy) => {
    ...
    if (proxy) {
        if (proxy.startsWith('socks5://')) {
            agent = new SocksProxyAgent(proxy); // SOCKS5 proxy
        } else {
        ...
        agent = ignoreSslAgent; // Default agent (ignores SSL issues)
    }

    return axios.create({ httpsAgent: agent });
};

const fetchPoints = async (headers, appId, axiosInstance) => {
    try {
        const response = await axiosInstance.get(`${apiEndpoints.getPoints}?appid=${appId}`, { headers });
...
const keepAliveRequest = async (headers, email, appId, axiosInstance) => {
    const payload = {
        username: email,
    ...
    try {
        const response = await axiosInstance.post(`${apiEndpoints.keepalive}?appid=${appId}`, payload, { headers });
...
const processAccount = async (account, proxy, appIds) => {
    ...
    const points = await fetchPoints(headers, appId, axiosInstance);
    ...
    const success = await keepAliveRequest(headers, email, appId, axiosInstance);
    if (success) {
